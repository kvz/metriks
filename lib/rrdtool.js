...
RRDTool.prototype.exe = function(cmd, options, cb) {
  ...
  exec(fullCmd, opts, function (err, stdout, stderr) {
    if (err !== null || stderr) {
      return cb('Error while executing ' + fullCmd + '. ' + err + '. stderr: ' + stderr);
    }
...
RRDTool.prototype.info = function (rrdFile, options, cb) {
  ...
  if (!fs.existsSync(rrdFile)) {
    ...
    // console.log('rrd file doesn not exist (yet) ' + rrdFile);
    return cb(null, null);
  }
  ...
  self.exe('info', options, function (err, stdout) {
    ...
    var info = unflatten(flat);

    if (!('ds' in info)) {
      console.log({rrdFile: rrdFile, info: info, stdout: stdout});
      return cb(thong.sprintf('No ds found in info for %s', rrdFile));
    }
...
/**
 * Numeric is something else than Number. We also allow posing strings.
...
RRDTool.prototype.toTitle = function (str) {
