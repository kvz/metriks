...
Metriks.prototype.graph = function (pattern) {
  ...
  self._loadAllPlugins(function(err, plugins) {
    ...
    self._writePNG(plugin, function (err) {
      ...
      self.cli.info('open %s', plugin.pngFile);
    });
  });
};
...
Metriks.prototype._writePNG = function (plugin, cb) {
  ...
  async.waterfall([
    function(callback) {
      ...
      self.cli.info('Creating directory %s', pngDir);
  ...
  self._loadAllPlugins(function(err, plugins) {
    ...
    plugins.forEach(function(plugin) {
      ...
      if (!plugin.executable) {
        self.cli.debug('Skipping plugin %s as it is not executable', plugin.name);
        return;
      }
      if (!plugin.enabled) {
        self.cli.debug('Skipping plugin %s as it is not enabled', plugin.name);
        return;
      }
      ...
      self._q.drain = function() {
        self.cli.debug('waiting for new items to be pushed to queue');
      };
      self._q.push(plugin);
    });
  });
...
Metriks.prototype._writeRRD = function(plugin, cb) {
  ...
  async.waterfall([
    function(callback) {
      ...
      self.cli.info('Creating directory %s', rrdDir);
    ...
    function(plugin, callback){
      ...
      exec(plugin.pluginFile, opts, function (err, stdout, stderr) {
        ...
        if (stderr) {
          self.cli.error('Saw stderr while running plugin: ' + stderr);
        }
...
Metriks.prototype._runPlugin = function(self, plugin, cb) {
  ...
  async.waterfall(tasks, function (err, output) {
    if (err) {
      return self.cli.error('failure %s.', err);
    }
    self.cli.info('%s task(s) for plugin %s complete', tasks.length, plugin.name);
    cb(null);
  });
...
Metriks.prototype._loadPlugin = function(pluginFile, cb) {
  ...
  self.cli.info('Loading plugin %s', pluginFile);
