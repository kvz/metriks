...
var Thong   = require('./thong').Thong;
var thong   = new Thong({out: console.log});
var RRDTool = require('./rrdtool').RRDTool;
var rrdtool = new RRDTool();
var unflatten = require('flat').unflatten;

exports        = module.exports = new Metrik();
exports.Metrik = Metrik;

function Metrik(config) {
...
Metrik.prototype.search = function (pattern, plugins) {
  var found = null;
...
Metrik.prototype.graph = function (pattern) {
  var self = this;
  self._loadAllPlugins(function(err, plugins) {
    ...
    self._writePNG(plugin, function (err) {
      ...
      thong.info('open %s', plugin.pngFile);
    });
  });
};

Metrik.prototype._writePNG = function (plugin, cb) {
  var self = this;
  ...
  async.waterfall([
    function(callback) {
      ...
      thong.info('Creating directory %s', pngDir);
    ...
    function (plugin, callback) {
      rrdtool.info(plugin.rrdFile, [], function(err, info) {
        ...
        options.push('--color'); options.push(thong.sprintf('BACK%s', '#1D1D1DFF'));
        options.push('--color'); options.push(thong.sprintf('CANVAS%s', '#141414FF'));
        options.push('--color'); options.push(thong.sprintf('SHADEA%s', '#282A2AFF'));
        options.push('--color'); options.push(thong.sprintf('SHADEB%s', '#121212FF'));
        options.push('--color'); options.push(thong.sprintf('GRID%s', '#232323FF'));
        options.push('--color'); options.push(thong.sprintf('MGRID%s', '#232323EE'));
        options.push('--color'); options.push(thong.sprintf('FRAME%s', '#2A1F1CFF'));
        options.push('--color'); options.push(thong.sprintf('FONT%s', '#FFFFFFEE')); // options.push(thong.sprintf('FONT%s', '#2A1F1CFF'));
        options.push('--color'); options.push(thong.sprintf('AXIS%s', '#4A4A4AFF'));
        ...
        dss.forEach(function (key) {
          ...
          options.push(thong.templ('DEF:{id}={rrdFile}:{ds}:AVERAGE', dsParams));
          options.push(thong.templ('LINE1:{id}{col}:"{descr}\\l"', dsParams));
        });
...
Metrik.prototype.start = function (rrdFile) {
  var self = this;
  self._loadAllPlugins(function(err, plugins) {
    if (err) {
      throw err;
    ...
    plugins.forEach(function(plugin) {
      ...
      if (!plugin.executable) {
        thong.debug('Skipping plugin %s as it is not executable', plugin.name);
        return;
      }
      if (!plugin.enabled) {
        thong.debug('Skipping plugin %s as it is not enabled', plugin.name);
        return;
      }
      ...
      self._q.drain = function() {
        thong.debug('waiting for new items to be pushed to queue');
      };
      self._q.push(plugin);
    });
  });
 ...
 * Clear all timers
 */
Metrik.prototype.stop = function () {
 ...
 * @return {object}
 */
Metrik.prototype._toSeries = function(plugin, stdout, stderr, cb) {
  var self   = this;
  var series = [];
  var cnt    = 0;
 ...
 * @param {object} plugin
 */
Metrik.prototype._writeRRD = function(plugin, cb) {
  var self = this;
  ...
  async.waterfall([
    function(callback) {
      ...
      thong.info('Creating directory %s', rrdDir);
    ...
    function(plugin, callback){
      ...
      exec(plugin.pluginFile, opts, function (err, stdout, stderr) {
        ...
        if (stderr) {
          thong.error('Saw stderr while running plugin: ' + stderr);
        }
    ...
    function(plugin, stdout, stderr, callback){
      ...
      Metrik.prototype._toSeries(plugin, stdout, stderr, function (err, series) {
        if (err) {
          return callback(err);
        }
    ...
    function(plugin, series, info, callback){
      ...
      series.forEach(function (item) {
        options.push(thong.templ('DS:{key}:GAUGE:600:U:U', {
          key: item.key
        }));
        values.push(item.value);
      });

      options.push(thong.templ('RRA:AVERAGE:0.5:1:300'));
 ...
 * @param  {[type]} plugin
 */
Metrik.prototype._runPlugin = function(self, plugin, cb) {
  thong.info(
  ...
  async.waterfall(tasks, function (err, output) {
    if (err) {
      return thong.error('failure %s.', err);
    }
    thong.info('%s task(s) for plugin %s complete', tasks.length, plugin.name);
    cb(null);
  });
...
Metrik.prototype._loadPlugin = function(pluginFile, cb) {
  // Parse options from source's comments
  var self         = this;
  var plugin      = {};
  plugin.graph      = {};

  thong.info('Loading plugin %s', pluginFile);
  ...
  exec(pluginFile, opts, function (err, stdout, stderr) {
    ...
    if (!plugin.rrdFile) {
      plugin.rrdFile = thong.sprintf('%s/%s-%s.rrd', plugin.name, os.hostname(), plugin.name);
    }
    ...
    if (!plugin.pngFile) {
      plugin.pngFile = thong.sprintf('%s/%s-%s.png', plugin.name, os.hostname(), plugin.name);
    }
 ...
 * @return {[type]}
 */
Metrik.prototype._loadAllPlugins = function (cb) {
  var self    = this;
  var plugins = [];

  if (!fs.existsSync(self.pluginDir)) {
    return cb(thong.sprintf('Plugin directory %s does not exist', self.pluginDir));
  }
