...
Plugin.prototype.reload = function(cb) {
  ...
  exec(self.pluginFile + ' config', opts, function (err, stdout, stderr) {
    ...
    _.extend(self.graph, self.defaultGraph, nested.graph);
    _.extend(self.graphStore, self.defaultGraphStore, nested.graphStore);

    // Allow complementing line defaults using wildcards
    if (nested.line) {
      _.extend(self.defaultLine, nested.line['*']);
      delete nested.line['*'];
    }
    if (nested.lineStore) {
      _.extend(self.defaultLineStore, nested.lineStore['*']);
      delete nested.lineStore['*'];
    }

    // Save line properties under dsName compatible names
    var cleanLine = {};
    _.each(nested.line, function(lineProperties, dsName) {
      // Merging defaults can only be done at runtime
      cleanLine[self.rrdtool.toDatasourceName(dsName)] = lineProperties;
    });
    self.line = cleanLine;

    // Save lineStore properties under dsName compatible names
    var cleanLineStore = {};
    _.each(nested.lineStore, function(lineProperties, dsName) {
      // Merging defaults can only be done at runtime
      cleanLineStore[self.rrdtool.toDatasourceName(dsName)] = lineProperties;
    });
    self.lineStore = cleanLineStore;
  ...
  async.waterfall([
    ...
    function(series, info, callback){
      ...
      series.forEach(function (item) {
        rrdCreateOptions.push(_.template('DS:{ dsName }:{ dsType }:{ heartBeat }:{ min }:{ max }')(self.getLineStore(item.dsName)));
...
Plugin.prototype.getLineStore = function (dsName) {
  var self = this;
  dsName   = self.rrdtool.toDatasourceName(dsName);
...
Plugin.prototype.getLine = function (dsName) {
  var self = this;
  dsName   = self.rrdtool.toDatasourceName(dsName);
    ...
    function (callback) {
      self.rrdtool.info(self.rrdFile, [], function(err, info) {
        ...
        _.keys(info.ds).forEach(function (dsName, lineIndex) {
          rrdGraphOptions.push(_.template('DEF:{ vName }={ rrdFile }:{ dsName }:{ consolidation }')(self.getLineStore(dsName)));
          rrdGraphOptions.push(_.template('{ element }:{ vName }{ color }:{ title }\\\\l')(self.getLine(dsName)));
        });
