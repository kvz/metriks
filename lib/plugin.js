...
Plugin.prototype.reload = function(cb) {
  ...
  exec(self.pluginFile + ' config', opts, function (err, stdout, stderr) {
    ...
    var cleanLines = {};
    _.each(self.graph.lines, function(properties, dsName) {
      cleanLines[self.rrdtool.toDatasourceName(dsName)] = properties;
    });
    ...
    function(series, info, callback){
      ...
      series.forEach(function (item) {
        ...
        _.extend(
          ...
          self.graph.lines['*'],
          self.graph.lines[item.dsName]
        );

        rrdCreateOptions.push(_.template('DS:{ dsName }:{ dsType }:{ heartBeat }:{ min }:{ max }')(lineParams));
        values.push(item.value);
      });

      rrdCreateOptions.push(_.template('RRA:{ consolidation }:{ xff }:{ step }:{ rows }')(rrdParams));
  ...
  async.waterfall([
    ...
    function (callback) {
      self.rrdtool.info(self.rrdFile, [], function(err, info) {
        ...
        var rrdGraphOptions = [];
        rrdGraphOptions.push(_.omit(self.graph, 'lines'));
        ...
        _.keys(info.ds).forEach(function (dsName, lineIndex) {
          ...
          rrdGraphOptions.push(_.template('DEF:{ vName }={ rrdFile }:{ dsName }:{ consolidation }')(lineParams));
          rrdGraphOptions.push(_.template('{ element }:{ vName }{ color }:{ title }\\\\l')(lineParams));
        });
