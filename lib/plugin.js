...
Plugin.prototype.getLineStore = function (dsName, lineIndex) {
  var self = this;
  dsName   = self.rrdtool.toDatasourceName(dsName);
...
Plugin.prototype.getLine = function (dsName, lineIndex) {
  var self = this;
  dsName   = self.rrdtool.toDatasourceName(dsName);
...
Plugin.prototype._uploadS3 = function (cb) {
  ...
  _.each(config, function(env, key) {
    ...
    if (!(v = process.env[env])) {
      return cb(new Error(
        'Please set a ' + env + ' environment var with the S3 ' + key
      ));
    }
    config[key] = v;
  });
  ...
  var files           = {};
  files[self.pngFile] = self.pngDir;
  files[self.rrdFile] = self.rrdDir;
    ...
    if (fs.existsSync(file)) {
      ...
      client.putFile(file, dst, headers, function(err, res){
        ...
        if (err || res.statusCode !== 200) {
          var msg = 'Error while uploading ' + file;
          if (res.statusCode) {
